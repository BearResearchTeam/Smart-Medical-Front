  ...
  <div class="ai-chat-container">
    ...
    <div class="chat-header">
      <div class="header-left">
        ...
        <span class="chat-title">AI 智能助手 (流式)</span>
      </div>
...
onMounted(() => {
  ...
  addMessage({
    id: generateId(),
    role: 'assistant',
    content: '您好！我是AI智能助手（流式版本），有什么可以帮助您的吗？',
    timestamp: Date.now()
  });
});
...
async function handleSendMessage() {
  ...
  try {
    ...
    // 发送原生流式请求
    const response = await cozeAIAPI.sendNativeStreamMessage({
    ...
    // 解析批量事件响应
    try {
      const events = JSON.parse(fullContent);
      if (Array.isArray(events)) {
        // 筛选出delta事件用于流式显示
        const deltaEvents = events.filter(event => event.event === 'conversation.message.delta');
        
        // 按时间戳排序
        deltaEvents.sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
        ...
        if (completedEvents.length > 0) {
          const finalEvent = completedEvents[0];
          
          // 更新conversationId
          if (finalEvent.conversationId) {
            conversationId.value = finalEvent.conversationId;
          }
          
          // 更新最终消息内容
          const finalMessage = messages.value.find(m => m.id === aiMessageId);
          if (finalMessage) {
            finalMessage.content = finalEvent.content;
          }
        }
      } else {
        // 如果不是数组格式，使用原来的处理方式
        throw new Error('响应格式不是事件数组');
      }
